...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (newScrolled && showShopDropdown) {
                            ...
                            >
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-sm text-gray-600">
                                  {item.quantity} x ${item.product.current_price.toFixed(2)}
                                </p>
                              </div>
                              <button
                                data-remove-item
        ...
        <div className="border-b relative hidden lg:block">
          <div className="container mx-auto px-4 flex justify-between items-center">
            <nav className="flex-1 flex items-center justify-between gap-8">
              ...
              >
                <button
                  className={`flex items-center gap-2 w-[15rem] p-4 border-b-2 border-transparent font-semibold transition-colors ${showCategories ? "bg-[#e94491]" : ""}`}
                >
                  <div className="relative w-5 h-5 flex items-center justify-center">
                    <div className="absolute inset-0 flex flex-col justify-center items-center gap-1">
                      <div
                        className={`w-4 h-0.5 bg-current transition-all duration-300 ${showCategories ? "rotate-45 translate-y-1.5 text-white" : ""}`}
                      ></div>
                      <div
                        className={`w-4 h-0.5 bg-current transition-all duration-300 ${showCategories ? "opacity-0" : ""}`}
                      ></div>
                      <div
                        className={`w-4 h-0.5 bg-current transition-all duration-300 ${showCategories ? "-rotate-45 -translate-y-1.5 text-white" : ""}`}
                      ></div>
                  ...
                  <span className={`transition-colors duration-300 ${showCategories ? "text-white" : ""}`}>
                    Browse Categories
                  </span>
                </button>
                              ...
                              <div className="space-y-1">
                                {shopCategories
                                  ...
                                  ?.subcategories.map((subcategory, index) => (
                                    <Link
                                      key={index}
                                      href={`/shop/${hoveredCategory.toLowerCase()}/${subcategory.toLowerCase().replace(/\s+/g, "-")}`}
                                      ...
                                      expandedSubMenus.has(`${item.name}-${subItem.name}`) && (
                                        <div className="ml-4 border-l-2 border-[#e94491] bg-white rounded-r-lg shadow-inner">
                                          {subItem.subcategories.map((subSubItem, subSubIndex) => (
                                            <Link
                                              key={subSubIndex}
                                              href={`/shop/${subItem.name.toLowerCase()}/${subSubItem.toLowerCase().replace(/\s+/g, "-")}`}
                          ...
                          {shopCategories.map((category, index) => (
                            ...
                            >
                              ...
                              >
                                <div className="flex items-center justify-between">
                                  <span>{category.name}</span>
                                  {category.subcategories.length > 0 && <ChevronRight className="h-3 w-3 opacity-60" />}
                                </div>
                              </Link>
                            </div>
                          ))}
